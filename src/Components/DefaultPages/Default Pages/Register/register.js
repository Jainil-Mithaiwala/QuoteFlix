...
function RegistrationPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      try {
        const emailCheckResponse = await axios.get(`https://quoteflix-register-login.onrender.com/check-email?email=${values.email}`);
        if (emailCheckResponse.data.exists) {
          setErrors({ email: 'Email already in use' });
          return;
        }

        const response = await axios.post('https://quoteflix-register-login.onrender.com/signup', values);
  ...
  return (
    <div className="wrapper">
      ...
      <div align="center" className='logintext'><a className='loginlink' href="/login_page">Log in</a></div>
    </div>
  );
}
